#!/usr/bin/env python3
# core025_member_trait_miner_v1__2026-03-26.py
#
# Full file. No placeholders.
# Separation-focused miner for 0025 / 0225 / 0255.
...
def build_member_separation_traits(core_hits: pd.DataFrame, min_support: int, min_dom_rate: float) -> pd.DataFrame:
    ...
    out = pd.DataFrame(rows)
    if len(out):
        out = out.sort_values(["winning_member_rate", "separation_gap", "support"], ascending=[False, False, False]).reset_index(drop=True)
    return out
...
def build_member_specific_traits(core_hits: pd.DataFrame, min_support: int, target_member: str, min_member_rate: float) -> pd.DataFrame:
    ...
    out = pd.DataFrame(rows)
    if len(out):
        out = out.sort_values(["target_member_rate", "support"], ascending=[False, False]).reset_index(drop=True)
    return out
...
def build_top2_needed_traits(pred_hits: pd.DataFrame, transitions: pd.DataFrame, min_support: int, min_top2_rate: float) -> pd.DataFrame:
    ...
    out = pd.DataFrame(rows)
    if len(out):
        out = out.sort_values(["top2_needed_rate", "support_top2_needed", "hit_event_support"], ascending=[False, False, False]).reset_index(drop=True)
    return out
...
def build_skip_danger_traits(pred_hits: pd.DataFrame, transitions: pd.DataFrame, min_support: int, min_skip_danger_rate: float) -> pd.DataFrame:
    ...
    out = pd.DataFrame(rows)
    if len(out):
        out = out.sort_values(["skip_danger_rate", "support_skipped_hits", "hit_event_support"], ascending=[False, False, False]).reset_index(drop=True)
    return out


def app():
    st.set_page_config(page_title="Core025 Member Trait Miner v1", layout="wide")
    st.title("Core025 Member Trait Miner v1")
    st.caption("Separation-focused miner for 0025 / 0225 / 0255. Mines member-dominance traits, Top2-needed pockets, and skip-danger pockets.")

    with st.sidebar:
        st.header("Trait mining controls")
        min_support = st.number_input("Minimum trait support", min_value=3, value=12, step=1)
        min_dom_rate = st.slider("Minimum dominance rate for separation traits", min_value=0.34, max_value=0.95, value=0.60, step=0.01)
        min_member_rate = st.slider("Minimum member rate for member-specific traits", min_value=0.34, max_value=0.95, value=0.60, step=0.01)
        min_top2_rate = st.slider("Minimum Top2-needed rate", min_value=0.05, max_value=0.95, value=0.30, step=0.01)
        min_skip_danger_rate = st.slider("Minimum skip-danger rate", min_value=0.05, max_value=0.95, value=0.30, step=0.01)
    ...
    st.download_button("Download separation traits CSV", df_to_csv_bytes(results["sep_traits"]), "core025_member_trait_miner_separation_traits__2026-03-26.csv", "text/csv")
    ...
    st.download_button("Download 0025 traits CSV", df_to_csv_bytes(results["traits_0025"]), "core025_member_trait_miner_0025_traits__2026-03-26.csv", "text/csv")
    ...
    st.download_button("Download 0225 traits CSV", df_to_csv_bytes(results["traits_0225"]), "core025_member_trait_miner_0225_traits__2026-03-26.csv", "text/csv")
    ...
    st.download_button("Download 0255 traits CSV", df_to_csv_bytes(results["traits_0255"]), "core025_member_trait_miner_0255_traits__2026-03-26.csv", "text/csv")
    ...
    st.download_button("Download Top2-needed traits CSV", df_to_csv_bytes(results["top2_needed"]), "core025_member_trait_miner_top2_needed_traits__2026-03-26.csv", "text/csv")
    ...
    st.download_button("Download skip-danger traits CSV", df_to_csv_bytes(results["skip_danger"]), "core025_member_trait_miner_skip_danger_traits__2026-03-26.csv", "text/csv")
    ...
    st.download_button("Download historical hit-event prediction table CSV", df_to_csv_bytes(results["pred_hits"]), "core025_member_trait_miner_hit_event_predictions__2026-03-26.csv", "text/csv")
